      ...
      <Provider store={store}>
        <div style={{backgroundColor:"white", zIndex:"10", border: "1px solid black", visibility:visibility?"":"hidden", width:window.innerwidth<300?"95%":"300px", position:"absolute", left:smallwindow?"":"665px", right:smallwindow?"0px":"", display:smallwindow?"block":"inline-block", margin:smallwindow?"0px":"0 30px"}}>
          <div className="chat-navbar">
            <button className="videobutton chat" style={{margin:"14px 12px", backgroundColor:this.state.isChatSelected?"papayawhip":"white"}} onClick={this.handleChatClick}></button>
            <button className="videobutton createpix" style={{margin:"14px 12px", backgroundColor:this.state.isCreateSelected?"papayawhip":"white"}} onClick={this.handleCreateClick}></button>
            <button className="videobutton listpix" style={{margin:"14px 12px", backgroundColor:this.state.isListSelected?"papayawhip":"white"}} onClick={this.handleListClick}></button>
          </div>
          {this.state.isChatSelected?<div>
            <div className="comment-container" id="commentcontainer" style={{height:"500px", overflow:"overlay", padding:"10px", backgroundColor:"ivory"}}>
              {
                ...
                ?list.map(post => {
                  ...
                  if(post.commentId){
                    ...
                    var tag = post.user.username.toLowerCase()
                    return (<div className="comment-bar" style={{backgroundColor: post.user.color}} key={post.id} id={post.id}>
                      <p><span>{post.user.profilePix?<PixBlock pix={post.user.profilePix} dim={16} adgrab={`profpic${post.id}`}/>:<img style={{height:"16px", width:"16px"}} src="/icons/userico.png"></img>} </span> <span>{tag}</span>: <div style={{border:"3px double black", display:"inline-flex"}}><PixBlock pix={post.pix} dim={64} adgrab={`comment${post.id}`}/></div></p>
                    </div>)                   
                  }
                })
                :null
            ...
            {this.props.user.id
              ?<div>
                <button className="videobutton abcchat" style={{margin:"0px 6px 0px 16px", backgroundColor:this.state.showPlippiBar?"white":"papayawhip"}} onClick={() => {this.setState({showPlippiBar:!this.state.showPlippiBar})}}/>
                <button className="videobutton plippi" style={{margin:"0px 16px 0px 6px", backgroundColor:this.state.showPlippiBar?"papayawhip":"white"}} onClick={() => {this.setState({showPlippiBar:!this.state.showPlippiBar})}}/>
